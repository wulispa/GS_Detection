...
PLT_API short __stdcall Plt_CardSetPrintfEnableFlag(uint16 enable_flag);
/*指令功能：开启/关闭函数调用打印功能
输入参数：  enable_flag  1: 开启打印  0：关闭打印
输出参数：	无
返回：      0或错误码*/
...
输出参数：  enable_flag: 1:开启此功能  0：关闭此功能
			...
			output_logic：输出电平
返回：      0或错误码*/
PLT_API short __stdcall Plt_IoSetInputFieterTime(uint16 cardid,uint16 bitno,double hold_time); 
...
{ 
  ...
  double home_low_vel;//回零高速， 单位：【pulse/s】,取值范围（0,4000000】
...
PLT_API short __stdcall Plt_CrdMoveLinerRel(uint16 cardid,uint16 crd,uint16 axis_num,uint16 *Axismaparray,double* targetpos_array);
/*指令功能：单段直线插补（相对运动）
输入参数：  cardid	卡号，取值范围:[0,11]。
            crd	坐标系号，取值范围:[0,1]。		
           axis_num	参与直线插补轴数
		   *Axismaparray	坐标系轴号列表
           *targetpos_array	参与直线插补各轴的相对位移（单位：PPU）
输出参数：无
返回值：0或错误码*/
PLT_API short __stdcall Plt_CrdMoveLinerAbs(uint16 cardid,uint16 crd,uint16 axis_num,uint16 *Axismaparray,double* targetpos_array);
/*指令功能：单段直线插补（绝对运动）
输入参数：  cardid	卡号，取值范围:[0,11]。
            crd	坐标系号，取值范围:[0,1]。	
            axis_num 轴数
...
输出参数：无
返回值：0或错误码*/
PLT_API short __stdcall Plt_CrdMoveCenterCircular(uint16 cardid,uint16 crd,uint16 *Axismaparray,struct_center_cir_parms cencirparms,double *targetpos_array,double *cenpos_array);
/*指令功能：单段圆心终点模式圆弧插补 
输入参数：  cardid	卡号，取值范围:[0,11]。
            crd	坐标系号，取值范围:[0,1]。	
            cencirparms 圆心终点方式圆弧参数
...
输出参数：无
返回值：0或错误码*/
PLT_API short __stdcall Plt_CrdMoveRadiusCircular(uint16 cardid,uint16 crd,uint16 *Axismaparray,struct_radius_cir_parms radcirparms,double *targetpos_array);
/*指令功能：单段终点半径模式圆弧插补 
输入参数： cardid	卡号，取值范围:[0,11]。
            crd	坐标系号，取值范围:[0,1]。	
            radcirparms 终点半径圆弧参数
...
输出参数：无
返回值：0或错误码*/
PLT_API short __stdcall Plt_CrdMove3PointCircular(uint16 cardid,uint16 crd,uint16 *Axismaparray,struct_3point_cir_parms pointscirparms,double *targetpos_array,double *midpos_array);
/*指令功能：单段三点模式圆弧插补 
输入参数：  cardid	卡号，取值范围:[0,11]。
            crd	坐标系号，取值范围:[0,1]。	
            radcirparms pointscirparms
...
输出参数：无
返回值：0或错误码*/
PLT_API short __stdcall Plt_CrdSetLimitSpeed(uint16 cardid,uint16 crd,double speed);
/*指令功能：设置插补极限速度
输入参数：  cardid	    卡号，取值范围[0,11]。
	        crd	坐标系号，取值范围:[0,1]。	
			speed	    轴的极限速度 ，单位：ppu/s
输出参数：	无
返回：      0或错误码*/
PLT_API short __stdcall Plt_CrdGetLimitSpeed(uint16 cardid,uint16 crd,double *speed);
/*指令功能：读取插补极限速度
输入参数：  cardid	    卡号，取值范围[0,11]。
	        crd	坐标系号，取值范围:[0,1]
输出参数：	*speed	    轴的极限速度 ，单位：ppu/s
...
//  输出参数：无
返回值：0或错误码*/
PLT_API short __stdcall Plt_IoCanInitAllOutput_extern(uint16 cardid,uint16 can_id,uint32 active_level);  	
...
PLT_API short __stdcall  Plt_GetTesterFinishFlag(uint16 cardid,uint16 tester_num,uint16 *finish_flag);
/*功能描述：检测测试仪是否完成一次测试
//  输入参数：cardid	        卡号，取值范围:[0,11]。 
              tester_num 0:测试站1  1：测试站2
//输出参数：  finish_flag   0:测试仪测试没有完成  1：测试完成
返回值：0或错误码*/
PLT_API short __stdcall  Plt_SampleTest(uint16 cardid,uint16 tester_num);
/*功能描述：测试仪样品测试命令
//  输入参数：cardid	        卡号，取值范围:[0,11]。 
    tester_num 0:测试站1  1：测试站2
返回值：0或错误码*/
PLT_API short __stdcall  Plt_SetTestDataFinisReadFlag(uint16 cardid,uint16 tester_num);
...
/*功能描述：写寄存器
//  输入参数：cardid	        卡号，取值范围:[0,11]。 
			 //address = 3800,   value= 1968,测盘从进料口转动到测试站2位置  
			 //address = 3800,   value= 1970,测盘从测试站2位置转动到进料口  
返回值：0或错误码*/
PLT_API short __stdcall  Plt_ReadRegister(uint16 cardid,uint32 address,long *value);
/*功能描述：读寄存器
//  输入参数：cardid	        卡号，取值范围:[0,11]。
              address= 6756 上位机控制测盘转动时，返回转动是否到位标志 value = 1 到位，0：没到位
//输出参数：value
返回值：0或错误码*/
PLT_API short __stdcall  Plt_ReadRegister_double(uint16 cardid,uint32 address,double *value);
/*功能描述：读寄存器
//  输入参数：cardid	        卡号，取值范围:[0,11]。
              address= 6756 上位机控制测盘转动时，返回转动是否到位标志 value = 1 到位，0：没到位
//输出参数：value
返回值：0或错误码*/
PLT_API short __stdcall  Plt_WriteRegister_double(uint16 cardid,uint32 address,double value);
/*功能描述：写寄存器(double型数据)
//  输入参数：cardid	        卡号，取值范围:[0,11]。 
			 //address = 3800,   value= 1968,测盘从进料口转动到测试站2位置  
			 //address = 3800,   value= 1970,测盘从测试站2位置转动到进料口  
返回值：0或错误码*/
PLT_API short __stdcall  Plt_read_diagnose_datas(uint16 cardid,uint16 datas_id,const char* pfilename);
/*功能描述：读取诊断数据
//  输入参数：cardid	        卡号，取值范围:[0,11]。
              datas_id          数据ID;
			  pfilename         存储数据文件
...
返回：      0或错误码*/
PLT_API short __stdcall Plt_EthercatSetNodeOd_INT(uint16 cardid,uint16 slave_id,uint16 Index,uint16 SubIndex,uint16 length,int32 Value);
...
输出参数：	无
返回：      0或错误码*/
PLT_API short __stdcall Plt_EthercatGetNodeOd_INT(uint16 cardid,uint16 slave_id,uint16 Index,uint16 SubIndex,uint16 length,int32 *Value);
/*指令功能：配置从站对象字典参数值(有符号)
输入参数：  cardid	卡号，取值范围[0,11]。
	        slave_id 从站id号
			Index 对象字典索引
...
输出参数：	无
返回：      0或错误码*/
PLT_API short __stdcall Plt_EthercatGetMasterState(uint16 cardid,uint16 *state);
/*指令功能：读取ethercat主站状态
输入参数：  cardid	卡号，取值范围[0,11]。
            slave_id 从站id号
...
输出参数：	无
返回：      0或错误码*/
PLT_API short __stdcall Plt_EthercatGetSlaveInfo(uint16 cardid,uint16 slave_id,uint16 info_type,uint8 *info);
/*指令功能：读取ethercat从站信息
输入参数：  cardid	卡号，取值范围[0,11]。
            slave_id 从站id号
...
//  输出参数：  *active_level
返回值：0或错误码*/
PLT_API short __stdcall Plt_EthercatGetAxisErrorCode(uint16 cardid,uint16 slave_id,uint32 *ErrorCode);  	
...
/*************************************************************************设置和读取下位机系统时间*********************************************************/
PLT_API short __stdcall  Plt_Set_System_Time(uint16 cardid,uint16 year,uint16 month,uint16 date,uint16 hour,uint16 min,uint16 sec);
/*功能描述：设置下位机系统 时间
  输入参数：  cardid	        卡号，取值范围:[0,11]。	
              year  年，范围[0,99]
			  month 月, 范围[1,12]
  ...
  输出参数：  无
返回值：0或错误码*/
PLT_API short __stdcall  Plt_Get_System_Time(uint16 cardid,uint16 *year,uint16 *month,uint16 *date,uint16 *hour,uint16 *min,uint16 *sec);
/*功能描述：设置下位机系统 时间
  输入参数：  cardid	        卡号，取值范围:[0,11]。	
  输出参数：  year  年，范围[0,99]
			  month 月, 范围[1,12]
