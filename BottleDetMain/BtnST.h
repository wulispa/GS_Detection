//
//	Add:添加了SetSpecialDisable()与ResetSpecialDisable()，使按钮被禁用时在外观上不会有任何变化，
//		借此实现一些特殊功能，例如用CButtonST来显示ICO格式的logo等
//
//	Class:		CButtonST
//
//	Compiler:	Visual C++
...
public:
	...
	void SetSpecialDisable();  //自己添加的函数，设置按钮被禁用的时候不变灰
	void ResetSpecialDisable();  //自己添加的函数，设置按钮被禁用的时候恢复变灰
...
private:
	...
	bool		m_bSpecialDisable;   // 自己添加的标志，指示按钮被禁用时是否变灰
