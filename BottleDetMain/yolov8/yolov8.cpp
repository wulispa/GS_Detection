...
YOLOV8::YOLOV8() :yolo::YOLO()
{
}

YOLOV8::YOLOV8(const dl_utils::InitParameter& param) :yolo::YOLO(param)
{
}
...
bool YOLOV8::init(const std::vector<unsigned char>& trtFile, const dl_utils::InitParameter& param)
{
    // input
    ...
    // binding dim
    if (param.dynamic_batch) // for some models only support static dynamic batch. eg: yolox
    {
    ...
    checkRuntime(cudaMalloc(&m_output_src_transpose_device, param.batch_size * m_output_area * sizeof(float)));
    // 4. cal affine matrix
...
void YOLOV8::preprocess(const std::vector<cv::Mat>& imgsBatch, const dl_utils::InitParameter& param)
{
    // 1.resize
    resizeDevice(param.batch_size, m_input_src_device, param.src_w, param.src_h,
        m_input_resize_device, param.dst_w, param.dst_h, 114, m_dst2src);

#if 0 // valid
    {
        float* phost = new float[3 * param.dst_h * param.dst_w];
        float* pdevice = m_input_resize_device;
        for (size_t j = 0; j < imgsBatch.size(); j++)
        {
            checkRuntime(cudaMemcpy(phost, pdevice + j * 3 * param.dst_h * param.dst_w,
                sizeof(float) * 3 * param.dst_h * param.dst_w, cudaMemcpyDeviceToHost));
            cv::Mat ret(param.dst_h, param.dst_w, CV_32FC3, phost);
            ret.convertTo(ret, CV_8UC3, 1.0, 0.0);
            cv::namedWindow("ret", cv::WINDOW_NORMAL);
            cv::imshow("ret", ret);
            cv::waitKey(1);
    ...
    // 2. bgr2rgb
    bgr2rgbDevice(param.batch_size, m_input_resize_device, param.dst_w, param.dst_h,
        m_input_rgb_device, param.dst_w, param.dst_h);
    ...
    // 3. norm:scale mean std
    normDevice(param.batch_size, m_input_rgb_device, param.dst_w, param.dst_h,
        m_input_norm_device, param.dst_w, param.dst_h, param);
            ...
            for (size_t y = 0; y < ret.rows; y++)
            {
                for (size_t x = 0; x < ret.cols; x++)
                {
                    for (size_t c = 0; c < 3; c++)
                    {
                        //
                        ret.at<cv::Vec3f>(y, x)[c]
                            = param.scale * (ret.at<cv::Vec3f>(y, x)[c] * param.stds[c] + param.means[c]);
                    }
    ...
    // 4. hwc2chw
    hwc2chwDevice(param.batch_size, m_input_norm_device, param.dst_w, param.dst_h,
        m_input_hwc_device, param.dst_w, param.dst_h);
#if 0
    {
        ...
        for (size_t j = 0; j < imgsBatch.size(); j++)
        {
            ...
            cv::Mat b(param.dst_h, param.dst_w, CV_32FC1, phost);
            cv::Mat g(param.dst_h, param.dst_w, CV_32FC1, phost + 1 * param.dst_h * param.dst_w);
            cv::Mat r(param.dst_h, param.dst_w, CV_32FC1, phost + 2 * param.dst_h * param.dst_w);
            std::vector<cv::Mat> bgr{ b, g, r };
            cv::Mat ret;
            cv::merge(bgr, ret);
            ret.convertTo(ret, CV_8UC3, 255, 0.0);
...
void YOLOV8::postprocess(const std::vector<cv::Mat>& imgsBatch, const dl_utils::InitParameter& param)
{
#if 0 // valid
    {
        float* phost = new float[param.batch_size * m_output_area];
        float* pdevice = m_output_src_device;
        for (size_t j = 0; j < imgsBatch.size(); j++)
        {
            checkRuntime(cudaMemcpy(phost, pdevice + j * m_output_area, sizeof(float) * m_output_area, cudaMemcpyDeviceToHost));
            //cv::Mat prediction(m_total_objects, param.num_class + 4, CV_32FC1, phost);
            cv::Mat prediction(param.num_class + 4, m_total_objects, CV_32FC1, phost); // for yolov8
             // write bin
            utils::saveBinaryFile(phost, 84 * m_total_objects, "d:/yolov8n.bin");
        }
        delete[] phost;
...
#endif // 0
    // transpose
    yolov8::transposeDevice(param, m_output_src_device, m_total_objects, 4 + param.num_class, m_total_objects * (4 + param.num_class),
        m_output_src_transpose_device, 4 + param.num_class, m_total_objects);
#if 0 // valid
    {
        float* phost = new float[m_total_objects * (4 + param.num_class)];
        float* pdevice = m_output_src_transpose_device;
        for (size_t j = 0; j < imgsBatch.size(); j++)
        {
            checkRuntime(cudaMemcpy(phost, pdevice + j * m_total_objects * (4 + param.num_class),
                sizeof(float) * m_total_objects * (4 + param.num_class), cudaMemcpyDeviceToHost));
            cv::Mat img_transpose(m_total_objects, 4 + param.num_class, CV_32FC1, phost);
        }
        delete[] phost;
    }
#endif // 0
    // decode
    yolov8::decodeDevice(param, m_output_src_transpose_device, 4 + param.num_class, m_total_objects, m_output_area,
        m_output_objects_device, m_output_objects_width, param.topK);
#if 0 // valid
    {
        float* phost = new float[1 + m_output_objects_width * param.topK];
        float* pdevice = m_output_objects_device;
        for (size_t j = 0; j < imgsBatch.size(); j++)
        {
            /* cv::imshow("srcimg", imgsBatch[j]);
             cv::waitKey(0);*/
            checkRuntime(cudaMemcpy(phost, pdevice + j * (1 + m_output_objects_width * param.topK),
                sizeof(float) * (1 + m_output_objects_width * param.topK), cudaMemcpyDeviceToHost));
            int num_candidates = phost[0];
            cv::Mat prediction(param.topK, m_output_objects_width, CV_32FC1, phost + 1);
        }
        delete[] phost;
    }
#endif // 0
    ...
    // nms
    //nmsDeviceV1(param, m_output_objects_device, m_output_objects_width, param.topK, param.topK * m_output_objects_width + 1);
    nmsDeviceV2(param, m_output_objects_device, m_output_objects_width, param.topK, param.topK * m_output_objects_width + 1, m_output_idx_device, m_output_conf_device);
#if 0 // valid
    {
        float* phost = new float[1 + m_output_objects_width * param.topK];
        float* pdevice = m_output_objects_device;
    ...
    // copy result from gpu to cpu
    checkRuntime(cudaMemcpy(m_output_objects_host, m_output_objects_device, param.batch_size * sizeof(float) * (1 + 7 * param.topK), cudaMemcpyDeviceToHost));

    // transform to source image coordinate,
    for (size_t bi = 0; bi < imgsBatch.size(); bi++)
    {
        int num_boxes = std::min((int)(m_output_objects_host + bi * (param.topK * m_output_objects_width + 1))[0], param.topK);
        for (size_t i = 0; i < num_boxes; i++)
        {
            float* ptr = m_output_objects_host + bi * (param.topK * m_output_objects_width + 1) + m_output_objects_width * i + 1;
            int keep_flag = ptr[6];
            if (keep_flag)
            {
                // yolov35678
                ...
                float y_rb = m_dst2src.v3 * ptr[2] + m_dst2src.v4 * ptr[3] + m_dst2src.v5;
                // yolov4
                //float x_lt = m_dst2src.v0 * ptr[0] * param.dst_w + m_dst2src.v1 * ptr[1] * param.dst_h + m_dst2src.v2; // left & top
                //float y_lt = m_dst2src.v3 * ptr[0] * param.dst_w + m_dst2src.v4 * ptr[1] * param.dst_h + m_dst2src.v5;
                //float x_rb = m_dst2src.v0 * ptr[2] * param.dst_w + m_dst2src.v1 * ptr[3] * param.dst_h + m_dst2src.v2; // right & bottom
                //float y_rb = m_dst2src.v3 * ptr[2] * param.dst_w + m_dst2src.v4 * ptr[3] * param.dst_h + m_dst2src.v5;
